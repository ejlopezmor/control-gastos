...
Instalación:
    pip install streamlit plotly pandas gspread google-auth
...
def get_resumen():
    ...
    for cat, (tipo, planeado) in data["presupuesto"].items():
        real = 0.0 if df.empty else df[df["categoria"] == cat]["monto"].sum()
...
with st.sidebar:
    ...
    st.progress(dia_actual / total_dias, text=f"Día {dia_actual} de {total_dias} del mes")
    st.divider()

    df_res         = get_resumen()
    df_ing         = get_df_ing()
    total_ing_real = df_ing["monto"].sum() if not df_ing.empty else 0
    ...
    st.metric("Ingresos reales", fmt_cop(total_ing_real),
              delta=fmt_cop(total_ing_real - total_ing_plan), delta_color="normal")
    st.metric("Gastos reales",   fmt_cop(total_real),
              delta=fmt_cop(total_real - total_planeado),     delta_color="inverse")
    st.metric("Balance del mes", fmt_cop(total_ing_real - total_real))

# ─────────────────────────────────────────────
# TABS
# ─────────────────────────────────────────────
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "🏠 Dashboard", "📊 Presupuesto vs Real",
    "📅 Gastos Diarios", "🔮 Proyección",
    "💳 Ingresar Gasto", "⚙️ Datos",
])
...
with tab1:
    ...
    ahorro_plan = df_res[df_res["Tipo"] == "Ahorro"]["Planeado"].sum()
    col4.metric("Ahorro ejecutado", fmt_cop(ahorro_real),
                delta=fmt_cop(ahorro_real - ahorro_plan), delta_color="normal")
    st.divider()

    if not desfasadas.empty:
        st.subheader("🚨 Categorías Desfasadas")
        for _, row in desfasadas.iterrows():
            st.error(f"**{row['Categoría']}** ({row['Tipo']}) — "
                     f"Planeado: {fmt_cop(row['Planeado'])} | Real: {fmt_cop(row['Real'])} | "
                     f"Exceso: **{fmt_cop(row['Real'] - row['Planeado'])}** ({row['% Ejecutado']:.0f}%)")
    else:
...
with tab2:
    ...
    df_plot  = (df_res if tipo_sel == "Todos" else df_res[df_res["Tipo"] == tipo_sel]).sort_values("% Ejecutado")

    fig = go.Figure()
    fig.add_trace(go.Bar(y=df_plot["Categoría"], x=df_plot["Planeado"], name="Planeado",
                         orientation="h", marker_color="#95a5a6", opacity=0.6))
    fig.add_trace(go.Bar(y=df_plot["Categoría"], x=df_plot["Real"],    name="Real",
                         orientation="h",
                         marker_color=["#e74c3c" if r > p else "#3498db"
                                       for r, p in zip(df_plot["Real"], df_plot["Planeado"])]))
    fig.update_layout(barmode="overlay", height=max(400, len(df_plot) * 40),
                      xaxis_title="Miles COP",
                      legend=dict(orientation="h", yanchor="bottom", y=1.02),
                      margin=dict(l=10, r=10, t=40, b=40), plot_bgcolor="white")
    st.plotly_chart(fig, use_container_width=True)
    st.caption("🔴 Rojo = superó presupuesto | 🔵 Azul = dentro del presupuesto")

    df_d = df_plot[["Tipo","Categoría","Planeado","Real","Diferencia","% Ejecutado"]].copy()
    for col in ["Planeado","Real","Diferencia"]:
        df_d[col] = df_d[col].map(lambda x: f"${x:,.0f}K")
    df_d["% Ejecutado"] = df_d["% Ejecutado"].map(lambda x: f"{x:.1f}%")
...
with tab3:
    ...
    if df.empty:
        ...
        df_dia = df.groupby("fecha")["monto"].sum().reset_index().sort_values("fecha")
        df_dia["acumulado"] = df_dia["monto"].cumsum()
        total_plan = get_resumen()["Planeado"].sum()
        df_dia["presupuesto_diario_acum"] = [
            total_plan / dias_en_mes() * (i + 1) for i in range(len(df_dia))
        ]

        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Gasto diario")
            fig1 = px.bar(df_dia, x="fecha", y="monto", color_discrete_sequence=["#3498db"],
                          labels={"monto": "Miles COP", "fecha": "Fecha"})
            fig1.update_layout(height=300, plot_bgcolor="white", margin=dict(t=20, b=20))
            st.plotly_chart(fig1, use_container_width=True)

        with col2:
            st.subheader("Acumulado vs. Ritmo presupuestado")
            fig2 = go.Figure()
            fig2.add_trace(go.Scatter(x=df_dia["fecha"], y=df_dia["presupuesto_diario_acum"],
                                      mode="lines", name="Ritmo presupuestado",
                                      line=dict(color="#95a5a6", dash="dash")))
            fig2.add_trace(go.Scatter(x=df_dia["fecha"], y=df_dia["acumulado"],
                                      mode="lines+markers", name="Gasto acumulado real",
                                      line=dict(color="#e74c3c", width=2),
                                      fill="tonexty", fillcolor="rgba(231,76,60,0.1)"))
            fig2.update_layout(height=300, plot_bgcolor="white", yaxis_title="Miles COP",
                               margin=dict(t=20, b=20),
                               legend=dict(orientation="h", yanchor="bottom", y=1.02))
            st.plotly_chart(fig2, use_container_width=True)

        st.subheader("Detalle de transacciones por día")
        fecha_sel = st.date_input("Ver transacciones del día:", value=df["fecha"].max().date())
        df_sel = df[df["fecha"].dt.date == fecha_sel]
        if df_sel.empty:
            st.info("Sin transacciones ese día.")
        else:
            st.dataframe(df_sel[["descripcion","categoria","medio","monto"]].rename(columns={
...
with tab4:
    ...
    if df.empty or dia_hoy == 0:
        ...
        col1.metric("Proyección gasto total",     fmt_cop(proyeccion_fin_mes),
                    delta=fmt_cop(proyeccion_fin_mes - total_plan_gastos), delta_color="inverse")
        col2.metric("Ritmo diario actual",        fmt_cop(ritmo_diario),
                    delta=f"Meta: {fmt_cop(total_plan_gastos / total_dias)}/día")
        col3.metric("Balance proyectado al cierre", fmt_cop(total_ing_plan - proyeccion_fin_mes))

        fechas_pasadas = pd.date_range(start=df["fecha"].min(), periods=dia_hoy, freq="D")
        acum_reales    = [df[df["fecha"] <= f]["monto"].sum() for f in fechas_pasadas]
        fechas_futuras = pd.date_range(start=date.today(), periods=total_dias - dia_hoy + 1, freq="D")
        acum_proy      = [gasto_hasta_hoy + ritmo_diario * i for i in range(len(fechas_futuras))]

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=fechas_pasadas, y=acum_reales,
                                 mode="lines+markers", name="Real",
                                 line=dict(color="#2ecc71", width=2)))
        fig.add_trace(go.Scatter(x=fechas_futuras, y=acum_proy,
                                 mode="lines", name="Proyección",
                                 line=dict(color="#e67e22", dash="dot", width=2)))
        fig.add_hline(y=total_plan_gastos, line_dash="dash", line_color="red",
                      annotation_text=f"Presupuesto: {fmt_cop(total_plan_gastos)}",
                      annotation_position="top left")
        fig.update_layout(height=380, plot_bgcolor="white",
                          yaxis_title="Gasto acumulado (K COP)", xaxis_title="Fecha",
                          legend=dict(orientation="h", yanchor="bottom", y=1.02),
                          margin=dict(t=40, b=40))
        st.plotly_chart(fig, use_container_width=True)

        st.subheader("⚠️ Categorías que se proyectan a desfasarse")
        alertas = []
        for _, row in df_res.iterrows():
            if row["Real"] > 0:
                proy = row["Real"] / dia_hoy * total_dias
                if proy > row["Planeado"]:
                    alertas.append({"Categoría": row["Categoría"], "Tipo": row["Tipo"],
...
with tab5:
    ...
    with col_gasto:
        ...
        with st.form("form_gasto"):
            ...
            cat_g    = st.selectbox("Categoría", sorted(data["presupuesto"].keys()), key="fg_cat")
...
with tab6:
    ...
    pres_df   = pd.DataFrame([{"Tipo": v[0], "Categoría": k, "Planeado (K COP)": v[1]}
                               for k, v in data["presupuesto"].items()])
    pres_edit = st.data_editor(
        pres_df, use_container_width=True, num_rows="dynamic",
        column_config={
    ...
    if st.button("💾 Guardar presupuesto"):
        ...
        if usar_sheets:
            with st.spinner("Guardando..."):
                guardar_presupuesto(client, nuevo_pres)
        st.success("Presupuesto actualizado.")
        st.rerun()

    st.divider()
    st.subheader("Presupuesto de Ingresos")
    ing_df   = pd.DataFrame([{"Categoría": k, "Planeado (K COP)": v}
    ...
    if st.button("💾 Guardar ingresos presupuestados"):
        nuevo_ing = {row["Categoría"]: float(row["Planeado (K COP)"])
                     for _, row in ing_edit.iterrows()}
        data["ingresos_presupuesto"] = nuevo_ing
        if usar_sheets:
            with st.spinner("Guardando..."):
                guardar_ingresos_presupuesto(client, nuevo_ing)
